...
void* detect_model_number() {
	...
	while(1) {
		...
		if(!strncmp(prop_buf,"S7260",5)) {
			ALOGI("Setting property %s to GT-S7260",MODEL_PROP);
			property_set(MODEL_PROP,"GT-S7260");
			is_dual_sim = 0;
		}
		if(!strncmp(prop_buf,"S7262",5)) {
			ALOGI("Setting property %s to GT-S7262",MODEL_PROP);
			property_set(MODEL_PROP,"GT-S7262");
		}
		break;
	}
	return NULL;
